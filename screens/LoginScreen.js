...
export default class LoginScreen extends React.Component {
  ...
  async _FetchRememberMe() {
    // Récupération des informations de login si RememberMe est checked
    try {
  ...
  async _storeRememberMe(key) {
    // Sauvegarde des informations de login si RememberMe est checked
    try {
  ...
  login() {
    ...
    if (this.rememberMe.current.props.checked == true) {
      this._storeRememberMe(
        this.logInput.current._lastNativeText +
          "," +
          this.passInput.current._lastNativeText +
          ",1"
      );
    } else {
    ...
    fetchTimeout(
      ...
      3000,
      "Impossible de se connecter. Veuillez recommencer d'ici 30 secondes."
    )
  ...
  render() {
    return (
      <View style={styles.loginBackground}>
        <Image
          source={require("../assets/images/jimee.jpg")}
          resizeMode="contain"
        />
        {/*<Text style={styles.smallWelcomeText}>
          le p'tit pote qui te rend visible
        </Text>*/}
        <TextInput
        ...
        >
          <CheckBox
            ...
            ref={this.rememberMe}
          />
          <Text style={{ color: "#fff", marginTop: 17, marginLeft: -14 }}>
